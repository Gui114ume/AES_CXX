...
AesCypher256::AesCypher256(ConfigReader& config)
: m_config{config}
{
    getKey();
    verify_key_size();
...
void AesCypher256::getKey()
{
    m_key = new aes_key_t;
    m_key->arr_key = new byte[AES_KEY_256_SIZE];
    std::fstream in(m_config.key_filename(), std::ios::in | std::ios::binary );
    in.read((char*)(m_key->arr_key), AES_KEY_256_SIZE);
    m_key->key_length = in.gcount();
    std::cout << "AesCypher256::getKey() implemented" <<std::endl;
}

chunk_t& AesCypher256::apply(chunk_t& chunk)
{
    //std::cout << "clear chunk size: " << chunk.size << std::endl;
    Aes256::AddRoundKey(&chunk, &(m_key_tab->tab[0]));

    int i = 0;
    for(i = 4 ; i < 4 * get_Nr() ;)
    {

        Aes256::SubBytes(&chunk, Aes256::sbox);
    
        Aes256::ShiftRows(&chunk);

        Aes256::MixColumns(&chunk);

        Aes256::AddRoundKey(&chunk, &(m_key_tab->tab[i]));

        i += 4;
    }
    Aes256::SubBytes(&chunk, Aes256::sbox);
    Aes256::ShiftRows(&chunk);
    Aes256::AddRoundKey(&chunk, &(m_key_tab->tab[ 4 * get_Nr()]));
    return chunk;
}
...
void AesCypher256::make_key_sched()
{
    ...
    while( i < get_Nb() * (get_Nr() + 1) ) 
    {
        ...
        {
            Aes256::RotWord(temp);
            Aes256::SubWord(temp, Aes256::sbox); 
            for(int j = 0 ; j < 4 ; j++)
                temp[j] = (j == 0) ? temp[j] ^ Aes256::rcon[i/get_Nk()] : temp[j] ^ 0b00000000;
    ...
    std::cout << "AesCypher256::make_key_sched() implemented" <<std::endl;
}

void AesCypher256::verify_key_size()
{
    if( m_key->key_length != AES_KEY_256_SIZE)
        {
            std::cout << "m_key->key_length != AES_KEY_256_SIZE, try another keyfile" << std::endl;
            std::cout << m_key->key_length << " != " << AES_KEY_256_SIZE << std::endl;
            exit(-1);
        }
    std::cout << "AesCypher256::verify_key_size() implemented" <<std::endl;
}
